...
export default function AdvancedPlantIdentifierPage() {
  ...
  function simulateAnalysis(mockData = null) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setLoadingText("Generating adulteration risk report...");
        setTimeout(() => {
          if (mockData) {
            setResult(mockData);
          } else {
            // Default random result for actual file uploads to look real
            setResult({
              name: "Unknown Medicinal Plant",
              confidence: 88,
              isAdulterant: Math.random() > 0.5,
              realName: "Unverified Substitute"
            });
          }
          setStatus("success");
        }, 1200);
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-2 rounded border border-slate-100 bg-slate-50">
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-700">
                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div> ResNet50 Classifier
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 min-h-[600px] flex flex-col">
              ...
              {status === 'success' && result && (
                <div className="flex-grow flex flex-col animate-in fade-in duration-500">
                  ...
                  {/* ADULTERANT WARNING (The core "Pivot" feature) */}
                  {result.isAdulterant ? (
                    <div className="mb-8 p-5 rounded-xl border-2 border-red-200 bg-red-50 relative overflow-hidden">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-red-800 mb-1">⚠️ HIGH ADULTERATION RISK DETECTED</h4>
                          <p className="text-xs text-red-700 leading-relaxed mb-3">
                            This specimen exhibits morphological traits consistent with <strong className="font-bold">{result.realName}</strong>, which is a common commercial adulterant or toxic look-alike for {result.name}.
                          </p>
                          <div className="bg-white/60 p-3 rounded-lg border border-red-100">
                            <p className="text-[10px] font-bold text-slate-800 uppercase tracking-widest mb-2">Required Human Verification Steps:</p>
                            <ul className="text-xs text-slate-700 space-y-1.5 list-disc pl-4">
                              <li>Crush a leaf; genuine {result.name} lacks the pungent odor of the substitute.</li>
                              <li>Examine the leaf margins carefully under light for micro-serrations.</li>
                    ...
                    <div className="mb-8 p-5 rounded-xl border-2 border-emerald-200 bg-emerald-50 relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-emerald-800 mb-1">GENUINE SPECIMEN PROBABLE</h4>
                          <p className="text-xs text-emerald-700">No high-risk adulterant traits detected in this scan. Proceed with standard quality checks.</p>
                        </div>
                      </div>
                    </div>
                  )}
